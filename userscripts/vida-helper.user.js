// ==UserScript==
// @name         VIDA Helper Inspector
// @namespace    https://vida.hmg.com/
// @version      0.1.1
// @description  Safe VIDA page inspector for building Mac/iPhone/browser helpers.
// @match        *://vida.hmg.com/*
// @match        *://vida.hmg.com:8081/*
...
(function vidaHelper() {
  ...
  function patchNetwork() {
    ...
    if (targetWindow.__vidaHelperNetworkPatched) return;
    targetWindow.__vidaHelperNetworkPatched = true;
    ...
    targetWindow.XMLHttpRequest.prototype.send = function patchedSend() {
      ...
      pushEvent({ type: "helper", method: "WARN", url: `network patch skipped: ${error && error.message || error}` });
    }
  }
  ...
  function cssPath(element) {
    ...
    while (node && node.nodeType === Node.ELEMENT_NODE && parts.length < 5) {
      ...
      for (const attr of stableAttrs) {
        ...
        if (value) {
          part += `[${attr}="${escapeCss(value)}"]`;
          break;
        }
      }
      if (node.classList && node.classList.length && !part.includes("[")) {
        part += "." + Array.from(node.classList).slice(0, 2).map((item) => escapeCss(item)).join(".");
      }
      parts.unshift(part);
